...
from bpmult.models.position_embedding import SinusoidalPositionalEmbedding
from bpmult.models.multihead_attention import MultiheadAttention
import math
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
                 embed_dropout=0.0, attn_mask=False, biprojection=False):
        super().__init__()
        ...
        self.biprojection = biprojection
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
                 attn_mask=False, biprojection=False):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        if self.biprojection:
            mask = buffered_future_mask(x, None) if self.attn_mask else None
        else:
            mask = buffered_future_mask(x, x_k) if self.attn_mask else None
        if x_k is None and x_v is None:
            x, _ = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
        elif self.biprojection:
            x, _ = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
            ...
            residual = x
            mask = buffered_future_mask(x, x_k) if self.attn_mask else None
            x_k = self.maybe_layer_norm(1, x_k, before=True)
            x_v = self.maybe_layer_norm(1, x_v, before=True)
